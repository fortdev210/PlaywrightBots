...
function save_options() {
  ...
  for (
    var i = 0;
    i <
    document.querySelectorAll('input[name="supplier_choice"][type="checkbox"]')
      .length;
    i++
  ) {
    if (
      document.querySelectorAll(
        'input[name="supplier_choice"][type="checkbox"]'
      )[i].checked === true
    ) {
      enabledSuppliers.push(
        document.querySelectorAll(
          'input[name="supplier_choice"][type="checkbox"]'
        )[i].value
      );
  ...
  ready(function () {
    ...
    for (var i = 0; i < enabledSuppliers.length; i++) {
      document.querySelector(
        '[name="supplier_choice"][value="' + enabledSuppliers[i] + '"'
      ).checked = true;
    }
